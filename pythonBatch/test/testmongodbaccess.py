...
def test_ok():
    """test_ok"""
    mongo_db_access_ok = MongoDBAccess(CONFIG)

    assert mongo_db_access_ok.status()

def test_file_error():
    """test file error"""
    mongo_db_access = MongoDBAccess(CONFIG_ERROR)
    assert not mongo_db_access.status()

def test_error():
    """test_error"""
    mongo_db_access_error = MongoDBAccess(CONFIG_ERROR_MONGODB)

    assert not mongo_db_access_error.status()

def test_find_one_ok():
    """test_find_one_ok"""
    mongo_db_access = MongoDBAccess(CONFIG)
