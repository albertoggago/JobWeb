...
class FindRepeated(object):
    ...
    def finding_open_jobs(self, delete_database, ratio_aceptation=1.0):
        """ find all open jobs for determinate if are repeated """
        delete_database = True if delete_database is True else False
        self._logger.info("Access all elements without response in database")
        count_jobs = 0
        report = {}
        report["lines"] = []
        jobs_finding = self._mongo_db_access\
               .find("correoUrl", {"control":"CORPUS", "decision":""})
        for job_finding in jobs_finding:
            count_jobs += 1
            line_report = self.analyse_job(job_finding, delete_database, ratio_aceptation)
            report["lines"].append(line_report)
        report["count"] = count_jobs
        return report

    def analyse_job(self, job, delete_database, ratio_aceptation):
        """ analyse a job determinate """
        _id = job.get("_id", "")
        self._logger.info("analize job %s", _id)
        ...
        jobs_similar = self._mongo_db_access\
        ...
        for job_similar in jobs_similar:
            ...
            equals = self.calculate_equals(info_similar, ratio_aceptation)
            if equals:
                if delete_database:
    ...
    def compare_similar(self, job_origin, job_compare):
        ...
        self._logger.info("_id simmilar: %s", _id_compare)
        comparation = {}
    ...
    def calculate_equals(self, data, ratio_aceptation):
        """ this method determinate the level of ranking for delete something"""
        self._logger.info("calculate equals")
        ...
        return True if ratio / var_count >= ratio_aceptation else False


    def delete_logic_job(self, job):
        """ analyse a job determinate """
        _id = job.get("_id", "")
        self._logger.info("delete Logic job: %s", _id)
        if _id != "":
